...
public:
	...
	T *get_cpu_wo() { state_ = CPU_IS_NEW; return cpu_; } // do sth to host buffer
	T *get_gpu_wo() { state_ = GPU_IS_NEW; return gpu_; } // do sth to device buffer
	const T* get_cpu_ro() { if (state_ != CPU_IS_NEW) { Sync(); } return cpu_; } // read sth from host buffer, if host buffer is not the newer, sync from device buffer
	const T* get_gpu_ro() { if (state_ != GPU_IS_NEW) { Sync(); } return gpu_; } // read sth from device buffer
	T *get_cpu_rw() { get_cpu_ro(); return get_cpu_wo(); } // sync then do sth to host buffer
	T *get_gpu_rw() { get_gpu_ro(); return get_gpu_wo(); } // sync then do sth to device buffer
};

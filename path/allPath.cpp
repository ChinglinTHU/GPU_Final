	...
	for(int it = 0; it < iter; it++)
	{
		...
		for(int t = 0; t < time; t++)
		{
			...
			for (int r = sta_t; r <= end_t; r++)
			{
				...
				// cout<<"trans = "<<trans<<endl;
				trans /= 10;
				float panning = gaussianD(trans, 10.f)*10;
				// cout<<panning<<endl;
				w[r-sta_t] = gaussianD(float(r-t), 10.f)*panning;
				// cout<<"w[r] = "<<w[r-sta_t]<<endl;
				w_sum += w[r-sta_t];
			}
					...
					int N = 0;
						if(0<i && i<width-1 && 0<j && j<height-1)
						{
							N=8;
							// tmpPath[i][j][t] += 2*optPath[i-1][j-1][t];
							cons3 += wgt*optPoints[i-1][j-1][t];
							cons3 += wgt*optPoints[i-1][j][t];
							cons3 += wgt*optPoints[i-1][j+1][t];
							cons3 += wgt*optPoints[i][j-1][t];
							cons3 += wgt*optPoints[i][j+1][t];
							cons3 += wgt*optPoints[i+1][j-1][t];
							cons3 += wgt*optPoints[i+1][j][t];
							cons3 += wgt*optPoints[i+1][j+1][t];
					...
					float gamma = 2*lambda*w_sum+wgt*N+1;
					// tmpPath[i][j][t] = tmpPath[i][j][t]/gamma;
					// cout<<"cellPath:"<<endl<<cellPath[i][j][t]<<endl
					// <<"cons2:"<<endl<<cons2<<endl
					// <<"cons3:"<<endl<<cons3<<endl
